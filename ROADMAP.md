...
- **Real cost-cutting tool (BUILT):** the old simulated dice-roll is gone. `llm_cost.py` computes real $ cost from actual token usage on every live API call (published per-model pricing, documented as approximate/needs verifying against current rates), and `agent.py` now records it as a genuine ledger expense every cycle — closing a real honesty gap where `fixed_cost_per_cycle` was a config guess never actually deducted from balance. `real_cost_cutting.py` analyzes an agent's own recent LLM spend and, only when it's genuinely eating into survival margin, switches to a cheaper model and reports the real estimated saving. Tested through all four real states: no history (no-op), low cost (correctly no-op), high cost (correctly switches models and reports savings), and already-cheapest (correctly no-op, not a false "cut").
